...
import { 
  ...
  normalizeStarsValue
} from '@/modules/table/components/EditableCells';
...
// â”€â”€â”€ Inline Editable Cell Components â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
              ...
              <td key={rc.token} className="relative px-2 py-2 border-b border-border text-xs min-w-[100px] bg-white">
                {isCollapsed && collapsedSummary ? (
                  ...
                  <EditableAssigneesCell
                    ...
                    onRenamePerson={handleRenamePersonGlobal}
...
export function ProjectTable() {
  ...
  const tableHandlers = useProjectTableHandlers({
    ...
    setBulkMenuOpen: tableState.setBulkMenuOpen,
    bulkMenuOpen: tableState.bulkMenuOpen,
    setDragPreview: tableState.setDragPreview,
    setCommentsOpen: tableState.setCommentsOpen,
    setEditingColumnId: tableState.setEditingColumnId,
    setNewColumnDialog: tableState.setNewColumnDialog,
    ...
    handleUpdate: tableActions.handleUpdate,
    handleDelete: tableActions.handleDelete,
    handleToggleExpand: tableActions.handleToggleExpand,
    dispatch,
    state,
  });
  ...
  try {
    return (
      ...
      <TableTools
        ...
        onBulkMenuToggle={() => setBulkMenuOpen((v) => !v)}
        onSelectAll={() => { setSelectedRowIds(new Set(renderedProjectIds)); setBulkMenuOpen(false); }}
        onBulkIndent={async () => { for (const id of getSelectionOrdered()) { await handleIndent(id); } }}
        onBulkOutdent={async () => { for (const id of getSelectionOrdered()) { await handleOutdent(id); } }}
        onBulkDuplicate={() => { getSelectionOrdered().forEach((id) => handleDuplicateRow(id)); }}
        onBulkDelete={async () => {
        ...
        onAddColumn={() => openCreateDynamicColumnDialog(dynamicColumns.length, 'text')}
      />
      </div>
        ...
        >
          ...
          >
            ...
            <TableHeader
              ...
              dragColumnToken={dragColumnToken}
              editingColumnId={editingColumnId}
              editingColumnName={editingColumnName}
              columnMenuOpenFor={columnMenuOpenFor}
              columnTypePickerFor={columnTypePickerFor}
              columnOptionsEditorFor={columnOptionsEditorFor}
              columnOptionsDraft={columnOptionsDraft}
              remoteEditingByColumn={remoteEditingByColumn}
              dynamicDisplayLabelEs={dynamicDisplayLabelEs}
              onDragColumnToken={setDragColumnToken}
              onColumnMenuOpenFor={setColumnMenuOpenFor}
              onEditingColumnName={setEditingColumnName}
              onEditingColumnId={setEditingColumnId}
              onColumnTypePickerFor={setColumnTypePickerFor}
              onColumnOptionsEditorFor={setColumnOptionsEditorFor}
              onColumnOptionsDraft={setColumnOptionsDraft}
              onBeginEditDynamicColumnName={beginEditDynamicColumnName}
              onCommitEditDynamicColumnName={commitEditDynamicColumnName}
              onOpenCreateDynamicColumnDialog={openCreateDynamicColumnDialog}
              onHandleChangeDynamicColumnType={handleChangeDynamicColumnType}
              onHandleDuplicateDynamicColumn={handleDuplicateDynamicColumn}
              onHandleDeleteDynamicColumn={handleDeleteDynamicColumn}
              onHandleSaveDynamicColumnOptions={handleSaveDynamicColumnOptions}
              onSetMoveCopyColumnId={setMoveCopyColumnId}
              onSetMoveCopyTargetId={setMoveCopyTargetId}
              onSetMoveCopyAsCopy={setMoveCopyAsCopy}
              onSetMoveCopyQuery={setMoveCopyQuery}
              onSetColumnOrder={setColumnOrder}
              onPersistDynamicPositionsFromOrder={persistDynamicPositionsFromOrder}
            />
            <tbody>
              {/* Scheduled projects */}
      ...
      {/* Comments side panel */}
      <CommentsPanel
        commentsOpen={tableState.commentsOpen}
        setCommentsOpen={tableState.setCommentsOpen}
        comments={tableState.comments}
        commentDraft={tableState.commentDraft}
        setCommentDraft={tableState.setCommentDraft}
        linkUrlDraft={tableState.linkUrlDraft}
        setLinkUrlDraft={tableState.setLinkUrlDraft}
        linkTitleDraft={tableState.linkTitleDraft}
        setLinkTitleDraft={tableState.setLinkTitleDraft}
        commentsTargetProject={state.projects.find(p => p.id === tableState.commentsTaskId)}
        user={user}
        submitComment={submitComment}
        submitLinkComment={submitLinkComment}
      />
      ...
      <DynamicColumnsDialog
        newColumnDialog={tableState.newColumnDialog}
        setNewColumnDialog={tableState.setNewColumnDialog}
        dynamicDisplayLabelEs={dynamicDisplayLabelEs}
        submitCreateDynamicColumn={submitCreateDynamicColumn}
      />
      </div>
    );
  } catch (err) {
