        ...
        {value.length === 0 ? (
          ...
          <span className="flex flex-wrap gap-1">
            {value.map((person, idx) => (
              <span key={idx} className="inline-flex items-center gap-1 px-1.5 py-0.5 bg-person-1/10 text-person-1 rounded text-xs">
                {person}
              </span>
            ))}
          </span>
          ...
          {currentTags.length === 0 ? (
            ...
            <span className="flex flex-wrap gap-1">
              {currentTags.map((tag, idx) => (
                <span key={idx} className="inline-flex items-center gap-1 px-1.5 py-0.5 bg-accent-blue/10 text-accent-blue rounded text-xs">
                  {tag}
                </span>
              ))}
            </span>
        ...
        {showOptions && (
          ...
          >
            {options.map((option) => (
              ...
              >
                <span>{option}</span>
                {currentTags.includes(option) && <span className="text-xs">?</span>}
              </button>
            ))}
            <button
              className="w-full text-left px-2 py-1 text-sm text-person-1 hover:bg-person-1/10 rounded transition-colors"
